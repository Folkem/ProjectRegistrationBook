...
@section('title', 'Оновлення керівника')
...
@section('body')
    ...
    <div class="flex w-full h-5/6">
        <div class="w-1/4 h-auto flex flex-col gap-4 m-auto rounded-3xl overflow-hidden p-4 border-black border-2">
            <div class="text-3xl text-center">Оновлення керівника</div>
            @if(session()->has('message'))
                <div class="w-full p-2 text-center text-lg rounded-xl bg-green-400">
                    {{ session('message') }}
                </div>
            @endif
            <form action="{{ route('supervisors.update', $supervisor) }}" method="post" class="flex flex-col gap-4">
                @csrf
                @method('put')
                <div->
                    <label for="name" class="sr-only">П.І.Б.: </label>
                    <input type="text" name="name" id="name" placeholder="П.І.Б."
